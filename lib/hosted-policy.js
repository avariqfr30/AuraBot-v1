...
const { decodeEncryptionKey } = require('./data-crypto');
...
function resolveHostedConfig(env = process.env) {
    ...
    const dataEncryptionKey = required(env, 'AURA_DATA_ENCRYPTION_KEY');
    decodeEncryptionKey(dataEncryptionKey);
    const dataKeyId = String(env.AURA_DATA_KEY_ID || 'primary').trim();
    const previousDataEncryptionKey = String(env.AURA_PREVIOUS_DATA_ENCRYPTION_KEY || '').trim();
    const previousDataKeyId = String(env.AURA_PREVIOUS_DATA_KEY_ID || '').trim();
    if (Boolean(previousDataEncryptionKey) !== Boolean(previousDataKeyId)) {
        throw new Error('AURA_PREVIOUS_DATA_ENCRYPTION_KEY and AURA_PREVIOUS_DATA_KEY_ID must be configured together');
    }
    if (previousDataEncryptionKey) decodeEncryptionKey(previousDataEncryptionKey);
    const localModels = modelList(required(env, 'AURA_LOCAL_MODELS'));
    const cloudModels = modelList(env.AURA_CLOUD_MODELS);
    ...
    const primaryModel = required(env, 'AURA_PRIMARY_MODEL');
    if (![...localModels, ...cloudModels].includes(primaryModel)) {
        throw new Error('AURA_PRIMARY_MODEL must be listed in AURA_LOCAL_MODELS or AURA_CLOUD_MODELS');
    }
    if (/medgemma/i.test(primaryModel)) {
        throw new Error('AURA_PRIMARY_MODEL must be a conversational model, not MedGemma');
    }
    const requestedMedicalModel = String(env.AURA_MEDICAL_MODEL || '').trim();
    if (requestedMedicalModel && !localModels.includes(requestedMedicalModel)) {
        throw new Error('AURA_MEDICAL_MODEL must be listed in AURA_LOCAL_MODELS');
    }
    const medicalModel = requestedMedicalModel ||
        localModels.find((model) => /medgemma/i.test(model)) || primaryModel;
    ...
    return {
        ...
        dataEncryptionKey,
        dataKeyId,
        previousDataEncryptionKey,
        previousDataKeyId,
        maxPromptChars: positiveInteger(env, 'HOSTED_MAX_PROMPT_CHARS', 120000),
        inferenceMonthlyUnits: positiveInteger(env, 'HOSTED_INFERENCE_MONTHLY_UNITS', 2000000),
        searchMonthlyRequests: positiveInteger(env, 'HOSTED_SEARCH_MONTHLY_REQUESTS', 500),
        localModels,
        cloudModels,
        primaryModel,
        medicalModel,
